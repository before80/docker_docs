...
Cache target is defined using the same `type=TYPE[,KEY=VALUE]` syntax defined by [`cache_from`](https://docs.docker.com/reference/compose-file/build/#cache_from).
...
The separator `=` is preferred, but `:` can also be used. Introduced in Docker Compose version [2.24.1](https://docs.docker.com/compose/releases/release-notes/#2241). For example:
