...
> **Note**
>
> Docker Debug requires a [Pro, Team, or Business subcription](https://docs.docker.com/subscription/details/). You must [sign in]({{< ref "/manuals/DockerDesktop/Signin" >}}) to use this command.
...
- `install [tool1] [tool2]`: Add Nix packages from: https://search.nixos.org/packages, see [example](https://docs.docker.com/reference/cli/docker/debug/#managing-your-toolbox-using-the-install-command).
- `uninstall [tool1] [tool2]`: Uninstall Nix packages.
- `entrypoint`: Print, lint, or run the entrypoint, see [example](https://docs.docker.com/reference/cli/docker/debug/#understanding-the-default-startup-command-of-a-container-entry-points).
...
| `-c, --command` |         | Evaluate the specified commands instead of starting an interactive session, see [example](https://docs.docker.com/reference/cli/docker/debug/#running-commands-directly-eg-for-scripting). |
| `--host`        |         | Daemon docker socket to connect to. E.g.: `ssh://root@example.org`, `unix:///some/path/docker.sock`, see [example](https://docs.docker.com/reference/cli/docker/debug/#remote-debugging-using-the---host-option). |
