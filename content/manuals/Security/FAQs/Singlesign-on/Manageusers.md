...
Yes. Admins can [force users to authenticate with Docker Desktop]({{< ref "/manuals/Security/Foradmins/Enforcesign-in" >}}) using a registry key, `.plist` file, or `registry.json` file.
...
For detailed prerequisites and instructions on how to enable SSO, see [Configure Single Sign-on]({{< ref "/manuals/Security/Foradmins/Singlesign-on/Configure" >}}).
...
[SCIM]({{< ref "/manuals/Security/Foradmins/Provisioning/SCIM" >}}) is available to provide full synchronization with users and groups. When you auto-provision users with SCIM, the recommended configuration is to disable JIT so that all auto-provisioning is handled by SCIM.

Additionally, you can use the [Docker Hub API]({{< ref "/reference/APIreference/DockerHubAPI/DockerHubAPI" >}}) to complete this process.
...
To auto-provision users without JIT provisioning, you can use [SCIM]({{< ref "/manuals/Security/Foradmins/Provisioning/SCIM" >}}).
