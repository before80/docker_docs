...
# Install Docker Desktop on Debian - 在 Debian 上安装 Docker Desktop
...
- For a Gnome Desktop environment, you must also install AppIndicator and KStatusNotifierItem [Gnome extensions](https://extensions.gnome.org/extension/615/appindicator-support/).
  ...
  - 对于 Gnome 桌面环境，还需安装 AppIndicator 和 KStatusNotifierItem [Gnome 扩展](https://extensions.gnome.org/extension/615/appindicator-support/)。
...
1. Set up Docker's `apt` repository. See step one of [Install using the `apt` repository](https://docs.docker.com/engine/install/debian/#install-using-the-repository). 设置 Docker 的 `apt` 仓库。请参阅[使用 `apt` 仓库安装](https://docs.docker.com/engine/install/debian/#install-using-the-repository)的第一步。
...
3. Install the package with apt as follows: 使用 apt 安装该包：
