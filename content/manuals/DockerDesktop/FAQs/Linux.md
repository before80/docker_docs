...
​	Docker Desktop for Linux 使用 [VirtioFS](https://virtio-fs.gitlab.io/) 作为默认（也是目前唯一的）主机和 Docker Desktop 虚拟机之间文件共享的机制。为了避免需要提升权限并在不必要限制共享文件操作的前提下，Docker Desktop 将文件共享服务 (`virtiofsd`) 运行在用户命名空间（请参阅 `user_namespaces(7)`）中，并配置了 UID 和 GID 映射。因此，Docker Desktop 依赖主机的配置来启用当前用户使用子 ID 委托。为此，`/etc/subuid`（请参阅 `subuid(5)`）和 `/etc/subgid`（请参阅 `subgid(5)`）必须存在。Docker Desktop 仅支持通过文件配置的子 ID 委托。Docker Desktop 将当前用户的 ID 和 GID 映射到容器中的 0，并使用 `/etc/subuid` 和 `/etc/subgid` 中与当前用户相对应的第一个条目，为容器中的 0 以上 ID 设置映射。
