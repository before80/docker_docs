...
## [Prerequisites](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#prerequisites)
...
- The [containerd image store](https://docs.docker.com/desktop/containerd/) must be enabled
- Write permissions to the registry repository where the image is stored

## [Introduction to VEX](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#introduction-to-vex)
...
## [`vexctl` example](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#vexctl-example)
...
- `--product`
  ...
  Package URL (PURL) of the Docker image. A PURL is an identifier for the image in a standardized format, defined in the PURL [specification](https://github.com/package-url/purl-spec/blob/master/PURL-TYPES.rst#docker).Docker image PURL strings begin with a `pkg:docker` type prefix, followed by the image repository and version (the image tag or SHA256 digest). Unlike image tags, where the version is specified like `example/app:v1`, in PURL the image repository and version are separated by an `@`.

- `--subcomponents`

  PURL of the vulnerable package in the image. In this example, the vulnerability exists in an npm package, so the `--subcomponents` PURL is the identifier for the npm package name and version (`pkg:npm/express@4.17.1`).If the same vulnerability exists in multiple packages, `vexctl` lets you specify the `--subcomponents` flag multiple times for a single `create` command.You can also omit `--subcomponents`, in which case the VEX statement applies to the entire image.
...
- `--justification`
  ...
  Justifies the `not_affected` status label, informing why the product is not affected by the vulnerability. In this case, the justification given is `vulnerable_code_not_in_execute_path`, signalling that the vulnerability can't be executed as used by the product.In OpenVEX, status justifications can have one of the five possible values:`component_not_present``vulnerable_code_not_present``vulnerable_code_not_in_execute_path``vulnerable_code_cannot_be_controlled_by_adversary``inline_mitigations_already_exist`For more information about these values and their definitions, see [Status Justifications](https://github.com/openvex/spec/blob/main/OPENVEX-SPEC.md#status-justifications) in the OpenVEX specification.

- `--file`

  Filename of the VEX document output

## [Example JSON document](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#example-json-document)
...
## [Verifying VEX documents](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#verifying-vex-documents)
...
## [Attach VEX documents to images](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#attach-vex-documents-to-images)

When you've created a VEX document, you can attach it to your image in the following ways:

- Attach the document as an [attestation](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#attestation)
- Embed the document in the [image filesystem](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#image-filesystem)
...
### [Attestation](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#attestation)
...
### [Image filesystem](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#image-filesystem)
...
The downside with this approach is that you can't change or update the exception later. Image layers are immutable, so anything you put in the image's filesystem is there forever. Attaching the document as an [attestation](https://docs.docker.com/scout/how-tos/create-exceptions-vex/#attestation) provides better flexibility.
