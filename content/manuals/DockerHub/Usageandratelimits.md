...
1. Sign in to [Docker Hub](https://hub.docker.com/).
   ...
   If you want to download usage for all members of an organization, you must sign in to an account that is an owner for that organization. Otherwise, you can only view your own usage.

2. In Docker Hub, navigate to the [**Usage** page](https://hub.docker.com/usage).

3. In the drop-down, select whether to download your personal data or data for an organization.

4. In **From** and **To**, select a date range for the data.

5. Select **Send report to email** to have Docker email you a link to the data file. Note that email processing time may vary.
...
1. [View your Docker Hub usage](https://docs.docker.com/docker-hub/download-rate-limit/#view-docker-hub-usage).
...
- Some images are unlimited through the [Docker Sponsored Open Source](https://www.docker.com/blog/expanded-support-for-open-source-software-projects/) and [Docker Verified Publisher](https://www.docker.com/partners/programs) programs.
...
>
> Note that when these changes take effect, the following new definition of a pull request and limits will take effect:
>
> - A Docker pull request includes both a version check and any download that occurs as a result of the pull. Depending on the client, a `docker pull` can verify the existence of an image or tag without downloading it by performing a version check.
...
| [Authenticated users](https://docs.docker.com/docker-hub/download-rate-limit/#how-do-i-authenticate-pull-requests) | 200 pulls per 6 hour period          |
