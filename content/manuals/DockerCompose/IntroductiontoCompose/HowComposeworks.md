...
​	Compose 文件（或 `compose.yaml` 文件）遵循 [Compose 规范](https://docs.docker.com/reference/compose-file/) 提供的定义多容器应用程序的规则。这是 Docker Compose 对正式 [Compose 规范](https://github.com/compose-spec/compose-spec) 的实现。
...
​	您可以使用[片段](https://docs.docker.com/reference/compose-file/fragments/)和[扩展](https://docs.docker.com/reference/compose-file/extension/)来保持 Compose 文件的高效和易于维护。
...
​	多个 Compose 文件可以[合并](https://docs.docker.com/reference/compose-file/merge/)在一起，以定义应用程序模型。YAML 文件组合通过基于指定的 Compose 文件顺序附加或覆盖 YAML 元素来实现。简单属性和映射会被最高优先级的 Compose 文件覆盖，列表则通过附加进行合并。相对路径基于第一个 Compose 文件的父文件夹解析。如果合并的辅助文件位于其他文件夹中，则相对路径按此规则处理。更多信息，请参阅[使用多个 Compose 文件](https://docs.docker.com/compose/how-tos/multiple-compose-files/)。
...
​	如果您希望重用其他 Compose 文件，或将应用程序模型的部分功能分离到单独的 Compose 文件中，可以使用 [`include`](https://docs.docker.com/reference/compose-file/include/)。这在您的 Compose 应用程序依赖于其他团队管理的应用程序或需要与他人共享时非常有用。
...
​	有关所有 Compose CLI 命令的完整列表，请参见[参考文档](https://docs.docker.com/reference/cli/docker/compose/)。
...
- [Quickstart 快速入门](https://docs.docker.com/compose/gettingstarted/)
- [Explore some sample applications 探索一些示例应用程序](https://docs.docker.com/compose/support-and-feedback/samples-for-compose/)
- [Familiarize yourself with the Compose Specification 熟悉 Compose 规范](https://docs.docker.com/reference/compose-file/)
