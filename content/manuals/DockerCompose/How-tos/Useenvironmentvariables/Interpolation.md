...
- This method is useful if you want to temporarily override an
  ...
  file that is already referenced in your
  ...
  file. For example you may have different
  ...
  ). In the following example, there are two environment files,
  ...
  . Both have different values set for
  ...
  is not used in the command line, the
  ...
  When an invalid file path is being passed as an
...
- You can use multiple
  ...
  options to specify multiple environment files, and Docker Compose reads them in order. Later files can override variables from earlier files.
